...
| HW エンコード | ✅ h264_amf(GPU 実測済み) | ✅ h264_videotoolbox 選択確認済み。稼働は間接証拠のみ(CPU 時間が libx264 比で約半分、-12903 の観測で VT セッション API 呼び出しを実証)。`sudo powermetrics` 直接確認は未実施 |
...
Mac の media-core 実機検証(§4-2)は 2026-07-12 に完了。残るのは `sudo powermetrics` での HW エンコード
直接確認と tauri dev の GUI 通し確認のみ(§4-2 参照)。次点は §4-3(初回リリース)/ §4-4(Phase 3)。
...
### 4-1. macOS CI ジョブの追加(小)— 完了(PR #58、マージ待ち)
.github/workflows/ci.yml に macos-latest ジョブ(brew ffmpeg + cargo build/test + pnpm test)を追加。
PR #58 で全15チェック green(マージはユーザー判断待ち)。macos-latest ランナーの brew ffmpeg は
8.1.2 で videotoolbox 有効。

### 4-2. media-core の Mac 実機検証 — 完了(2026-07-12。残: powermetrics 直接確認・GUI 通し確認)
...
2. ⚠️ HW 動作確認は間接証拠のみ: CPU 時間が libx264 比で約半分(user 11.4s vs 21.8s)、-12903 エラーの
   観測で VideoToolbox セッション API を実際に叩いていることは実証済み。`sudo powermetrics` での直接
   確認は未実施(人間の手作業として残る)
...
6. tauri dev でアプリを起動し、読み込み→編集→書き出し→アップロード画面の通し確認は人間の手作業として
   残る(起動スモークテストは別途実施中)
