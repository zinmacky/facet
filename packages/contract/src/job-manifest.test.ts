...
describe("jobManifest", () => {
  ...
  it("idempotencyKey が UUID でなければ弾く", () => {
    expect(jobManifest.safeParse({ ...valid, idempotencyKey: "not-a-uuid" }).success).toBe(false);
  });

  it("caption 2200 文字超を弾く", () => {
    expect(jobManifest.safeParse({ ...valid, caption: "x".repeat(2201) }).success).toBe(false);
  });

  it("platform は instagram 固定(youtube は経路が別なので不可)", () => {
    expect(jobManifest.safeParse({ ...valid, platform: "youtube" }).success).toBe(false);
  });

  it("publishAt は正の整数のみ", () => {
    expect(jobManifest.safeParse({ ...valid, publishAt: -1 }).success).toBe(false);
    expect(jobManifest.safeParse({ ...valid, publishAt: 1.5 }).success).toBe(false);
  });
});
