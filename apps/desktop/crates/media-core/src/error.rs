...
pub enum MediaError {
	...
	/// 実行中の別ジョブと入出力パスが交差している(クロスジョブのパス競合防止)。
	///
	/// [`OutputBusy`](Self::OutputBusy)は「同一 `output_path` への同時書き込み」
	/// (=完全一致)のみを検知するのに対し、こちらは「自分の出力先が、実行中の別ジョブが
	/// 読み取り中の入力と一致する」または「自分の入力が、実行中の別ジョブが書き込み中の
	/// 出力先と一致する」の 2 パターンを指す。いずれも `pipeline::finalize_output` の
	/// `fs::rename` が、別ジョブの demuxer がハンドルを開いたままのファイルを上書きする
	/// (または逆に、別ジョブがまさにリネームしようとしているファイルを読み取る)destructive
	/// race であり、`OutputBusy` と同様に実際の書き出し処理へは一切入らず即座に失敗する。
	/// `OutputBusy` と異なり「待てば解決する同一リクエストの重複」ではなく「そもそも別々の
	/// ファイルを指すべき2つのパスが交差している」設定ミスに近いため、
	/// `preview::coalesce_render` の待機・再試行の対象にはしない(同モジュールの
	/// `coalesce_render` 冒頭コメント参照)。
	#[error("パスが実行中の別ジョブと競合しています(データ損失を防ぐため中断しました): {path}")]
	CrossJobPathConflict { path: PathBuf },
