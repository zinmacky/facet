...
/** startExport の 1 clip 版(Harness を clip 1 本で使うテスト用)。 */
async function startExportSingleClip(user: ReturnType<typeof userEvent.setup>) {
	mockDialogOpen.mockResolvedValueOnce("/out");
	await user.click(screen.getByRole("button", { name: /書き出しを開始/ }));
...
describe("ExportScreen: 中央パネルの構成(UX変更3 — プレビュー常設 + 書き出し状態カードの分離表示)", () => {
	it("「書き出しを開始」後もクロップ内容プレビュー(生成/更新ボタン含む)が中央に表示され続ける", async () => {
		const user = userEvent.setup();
		const clipA = makeClip("clip-a", "clipA", 10);
		renderWithProviders(<Harness initialClips={[clipA]} />);

		// 開始前: プレビュー生成ボタンが中央に見える。
		expect(
			screen.getByRole("button", { name: "プレビュー生成" }),
		).toBeInTheDocument();

		await startExportSingleClip(user);

		// 以前は started で中央パネルがプレビュー⇔書き出し詳細に排他切り替えされ、
		// プレビュー生成ボタンへの導線が消えていた。今は started 後も同じボタンが
		// 使え続ける(内容確認はプレビュー、ファイル出力は書き出しで完全に分離)。
		expect(
			screen.getByRole("button", { name: "プレビュー生成" }),
		).toBeInTheDocument();
	});

	it("書き出し中/完了/エラーは中央下の状態カードに video 埋め込み無しで表示され、プレビューと共存する", async () => {
		const user = userEvent.setup();
		const clipA = makeClip("clip-a", "clipA", 10);
		renderWithProviders(<Harness initialClips={[clipA]} />);
		...
		// 実行中: 状態カードにのみ「書き出し中…」文言が出る(一覧側は % のみ表示)。
		expect(screen.getByText(/書き出し中…/)).toBeInTheDocument();
		// プレビュー側の生成ボタンは引き続き使える(状態カードと共存)。
		expect(
			screen.getByRole("button", { name: "プレビュー生成" }),
		).toBeInTheDocument();
		...
		expect(
			screen.getByRole("button", { name: "フォルダで表示" }),
		).toBeInTheDocument();
		expect(document.querySelector("video")).not.toBeInTheDocument();
	});

	it("書き出し開始前は状態カードを一切出さず、開始後にタスクの無い clip ではプレースホルダ1行のみを出す", async () => {
		const user = userEvent.setup();
		const clipA = makeClip("clip-a", "clipA", 10);
		renderWithProviders(<Harness initialClips={[clipA]} />);

		// 開始前: ExportDetail 自体をマウントしないため、状態文言は一切出ない
		// (右一覧も ExportPreviewListItem であり「未書き出し」文言を持たない)。
		expect(screen.queryByText(/未書き出し/)).not.toBeInTheDocument();
		expect(screen.queryByText(/要再書き出し/)).not.toBeInTheDocument();

		await startExportSingleClip(user);
		// clip-a は起動済み(running)のため、この時点でプレースホルダは出ない。
		expect(screen.queryByText(/未書き出し/)).not.toBeInTheDocument();

		// 開始後に新規追加された clip は task が無い(未書き出し)。中央には
		// 進捗バー/キャンセルボタンを伴わない1行のプレースホルダのみが出る
		// (右一覧側の StatusPill と合わせて文言は2箇所に出る)。
		await user.click(screen.getByRole("button", { name: "add-clip-c" }));
		await user.click(listRow("clipC.mp4"));

		await waitFor(() => {
			expect(screen.getAllByText(/未書き出し/)).toHaveLength(2);
		});
		expect(
