 ...
 * 折りたたみを開く。「この投稿をすべて投稿」ボタン(publishPostMutation 経由)を
 * 露出させるために使う。
 */
...
describe("App: 投稿処理中(stepLocked)はヘッダの元動画選択を禁止する", () => {
	...
	it("OutputPublishSection 単体の「投稿」ボタン経由でも busy/stepLocked が働く(HIGH バグの回帰: 以前はこの経路が publishPostMutation/publishAllMutation を経由しない fire-and-forget 呼び出しのため busy に反映されず、投稿中でも元動画を選び直せてしまっていた)", async () => {
		const user = userEvent.setup();
		await mockInvoke("set_youtube_oauth_client", {
			clientId: "client-id",
			clientSecret: "client-secret",
		...
		// post 単位の一括投稿(expandPostScheduleSettings)を経由せず、OutputPublishSection
		// 単体の「投稿」ボタンを直接クリックする。
		...
		// 投稿レンダリング(publish 品質の preview_start)が発火するところまで進める。
		// done イベントを送らないことで publishOutput を pending のまま止め、
		// uploadBusy(= stepLocked)を true に保つ。
		await waitFor(() =>
			expect(mockInvoke).toHaveBeenCalledWith(
				"preview_start",
		...
		// stepLocked: 他ステップへの遷移ボタンが無効化される。
		await waitFor(() =>
			expect(stepNav().getByRole("button", { name: /確認/ })).toBeDisabled(),
		);

		// ヘッダの「元動画を選択」ボタンも無効化されている。
		const pickButton = screen.getByRole("button", { name: "元動画を選択" });
		expect(pickButton).toBeDisabled();
