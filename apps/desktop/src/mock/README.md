...
- `path.ts` — `@tauri-apps/api/path` 差し替え(`join`/`documentDir`)。
- `dialog.ts` — `@tauri-apps/plugin-dialog` 差し替え(`open`)。
- `opener.ts` — `@tauri-apps/plugin-opener` 差し替え(`openPath`)。
- `notification.ts` — `@tauri-apps/plugin-notification` 差し替え
  (`isPermissionGranted`/`requestPermission`/`sendNotification`)。
- `updater.ts` — `@tauri-apps/plugin-updater` 差し替え(`check`)。常に固定の更新が
  「利用可能」として見つかったことにし、`UpdateNotification` バナーを目視確認できるように
  する(Phase 4 Wave C)。
- `process.ts` — `@tauri-apps/plugin-process` 差し替え(`relaunch`)。`console.log` のみ。
...
- `fixtures.ts` — 固定のダミーパス・probe 結果・サンプル動画 URL・更新情報。
...
- 通知(`sendNotification`): 権限は常に許可済み扱いで、`console.log` のみ。
- `set_max_concurrent_encodes`: no-op(dev:mock はジョブを実際には並列実行しないため)。
- `check`(更新確認): 起動のたびに固定バージョン(`MOCK_UPDATE_INFO`)を「利用可能」として
  返す。バナーの [更新する] → ダウンロード進捗(200ms 刻み、3s で完了)→ [再起動]
  (`relaunch` は `console.log` のみ)まで一通り確認できる。「後で」の 24h 抑制は
  localStorage 依存のため実際に日時を跨がないと再現できない(ロジック自体は
  `lib/updater.test.ts` で検証)。
