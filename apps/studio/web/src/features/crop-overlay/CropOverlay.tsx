 ...
 * 表示のみを担い、値は親に返す。
 */
export function CropOverlay({ crop, onChange, aspect, snap }: CropOverlayProps) {
  ...
  const beginResize = useCallback(
    (corner: Corner) => (e: ReactPointerEvent<HTMLDivElement>) => {
      ...
      const orig = crop;
      // 固定される反対側の角(正規化)。
      const anchorX = corner === "nw" || corner === "sw" ? orig.x + orig.width : orig.x;
      const anchorY = corner === "nw" || corner === "ne" ? orig.y + orig.height : orig.y;
...
function Scrim({ crop }: { crop: CropRect }) {
  ...
  return (
    <>
      <div className={box} style={{ left: 0, right: 0, top: 0, height: `${crop.y * 100}%` }} />
      <div
        className={box}
        style={{ left: 0, right: 0, top: `${(crop.y + crop.height) * 100}%`, bottom: 0 }}
      />
      <div
        className={box}
        style={{
