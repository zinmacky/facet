...
const MAX_CONCURRENT = Math.max(
	1,
	Number(process.env.MAX_CONCURRENT_ENCODES) || 2,
);
...
function isEncoderOpenError(err: unknown): boolean {
	...
	return /Error while opening encoder|Could not open encoder|err=-12903|-12903/.test(
		m,
	);
}
	...
	try {
		...
		try {
			...
			if (usesVideoToolbox(opts) && isEncoderOpenError(err)) {
				console.warn(
					`VideoToolbox 起動に失敗。libx264 で再試行します: ${opts.output}`,
				);
				onFallback?.();
